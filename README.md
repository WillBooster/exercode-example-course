...
| `canFinishExaminationEarly` | 真偽   | 試験の途中終了を**有効**にする。有効にすると、学生は「試験を終了する」ボタンで提出ハード締切（`submissionHardClosedAt`）より前に試験を終了できる。終了後は締切まで解答・提出ができなくなり、この操作は取り消せない。`isExamination` が有効で、かつ `submissionHardClosedAt` が設定されている場合のみ機能する。 |
