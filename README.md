...
| `stdin`      | 標準入力の入力欄を表示する。`input()` などで標準入力を読むプログラムに指定する。指定しない場合（直後の `stdin` ブロックもない場合）、学生は標準入力を使用できない。 |

実行できるコードブロックの直後に `stdin` を言語として指定したコードブロックを置くと、その内容が標準入力の入力欄の初期値になります。
